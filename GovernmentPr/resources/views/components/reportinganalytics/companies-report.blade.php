...
document.addEventListener('DOMContentLoaded', function () {
    ...
    downloadButton.addEventListener('click', function (event) {
        ...
        const currentPage = dataTable.page(); // Store current page
        const pageLength = dataTable.page.len(); // Store current page length

        // Show all records temporarily
        dataTable.page.len(-1).draw();

        // Give DataTables time to render all rows
        setTimeout(function () {
            const table = document.getElementById('companiesTable');
            const workbook = XLSX.utils.table_to_book(table, { sheet: 'Companies Data' });
            const excelFileName = 'companies_data.xlsx';
            XLSX.writeFile(workbook, excelFileName);

            // Restore original pagination
            dataTable.page.len(pageLength).draw();
            dataTable.page(currentPage).draw(false); // Return to previous page
        }, 500); // Delay to ensure table is fully rendered
    });
});
        </script>
