...
<div class="container py-5">
    ...
    <ul class="nav nav-tabs mb-3" id="guardTabs" role="tablist">
        @foreach($guards as $guard)
            @php $guardId = Str::slug($guard->title); @endphp
            <li class="nav-item" role="presentation">
                <button class="nav-link {{ $loop->first ? 'active' : '' }}" 
                        id="tab-{{ $guardId }}" 
                        data-bs-toggle="tab" 
                        data-bs-target="#guard-{{ $guardId }}" 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateRowToggle(roleId, guard) {
        ...
        rowToggle.indeterminate = !allChecked && someChecked;
    }
    ...
    function updateColToggle(permId, guard) {
        ...
        if (!colToggle) return;
        const allChecked = [...colCbs].every(cb => cb.checked);
        const someChecked = [...colCbs].some(cb => cb.checked);
        colToggle.checked = allChecked;
        colToggle.indeterminate = !allChecked && someChecked;
    }
